...
const Header = () => {
  ...
  const handleClick = (index: number) => {
    setFocusedButtonIndex(index === focusedButtonIndex ? null : index);
  };

  const buttonsData = [
    { text: "Features", icon: true },
    { text: "Solutions", icon: true },
    { text: "Plans", icon: true },
    { text: "Pricing", icon: false },
    { text: "Resources", icon: true },
  ];

  return (
    <header className="flex items-center hover:shadow-lg duration-500 px-[108px]">
      <a href="#" className="p-4 h-[60px]">
        <svg
      ...
      <div className="flex">
        {buttonsData.map((button, index) => (
          <button
            key={index}
            className={`flex items-center gap-1 p-4 pt-5 hover:text-[#0065ff] ${
              index === focusedButtonIndex
                ? "focus:underline focus:underline-offset-[20px] focus:text-[#0065ff]"
                : ""
            }`}
      ...
      <div className="flex items-stretch ml-auto">
        <Link
          to="/login"
          className="flex items-center align px-6 py-2 h-[60px] text-xl">
          Log in
        </Link>

        <a
          href="#"
          className="flex items-center px-6 py-2 h-[60px] text-xl bg-[#0065ff] text-white hover:bg-[#0052cc]">
          Get Trello for free
        </a>
      </div>
    </header>
